...
UAVS = ["vrx_hexrotor", "vrx_quadrotor"]
...
class Model:
    ...
    def bridges(self, world_name):
        ...
        if world_name == "robotx_2024_course_docking":
            for model in ["dock_2022", "robotx_light_buoy_rgb"]:
                bridges.extend(
                    [
                        vrx_gz.bridges.joint_states(world_name, model),
                    ]
                )
        elif world_name == "robotx_2024_course_a":
            models = [
                *[f"red_bound_{i}" for i in range(9)],
                *[f"green_bound_{i}" for i in range(9)],
                *[f"obstacle_{i}" for i in range(17)],
            ]
            for model in models:
                bridges.extend(
                    [
    ...
    def payload_bridges(self, world_name, payloads=None):
        ...
        bridges, nodes, payload_launches = self.payload_bridges_impl(
            world_name, payloads
        )
            ...
            if sensor_type == sdf.Sensortype.CAMERA:
                ...
                nodes.append(
                    Node(
                        ...
                        arguments=["1"],
                        remappings=[
                            ("input/image", f"{ros_sensor_prefix}/depth"),
    ...
    def custom_model_launch(self, world_name, model_name, model):
        custom_launch = None
        path = os.path.join(get_package_share_directory(model), "launch")
    ...
    def custom_payload_launch(self, world_name, model_name, payload, idx):
        payload_launch = None
        path = os.path.join(get_package_share_directory(payload), "launch")
        if os.path.exists(path):
    ...
    def xacro_cmd(self):
        ...
        model_dir = os.path.join(
            get_package_share_directory("vrx_gazebo"), "models", self.model_name
        )
